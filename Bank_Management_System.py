...
marvel_Bank = Bank("Bank_ragnarok")
Manager = Admin("Tanvir", "tanvir.rivu07@gmail.com", "dhaka")
Manager.createAdmin(Manager, marvel_Bank)
...
def adminMenu():
    ...
    while True:
        ...
        if Option == 1:
            ...
            if marvel_Bank.findAdmin(adminEmail):
                print("Sorry This Email Allready Exist")
            else:
                newAdmin = Admin(adminName, adminEmail, adminAddress)
                Manager.createAdmin(newAdmin, marvel_Bank)

        elif Option == 2:
            userEmail = input("Type User Email ")
            Manager.remove_User(userEmail, marvel_Bank)
        elif Option == 3:
            Manager.view_Admin_List(marvel_Bank)
        elif Option == 4:
            Manager.view_User_List(marvel_Bank)
        elif Option == 5:
            Manager.chackStock(marvel_Bank)
        elif Option == 6:
            Manager.checkLoanAmount(marvel_Bank)
        elif Option == 7:
            print(f"Loan Status\n On/Off")
            status = input("type on or off ").lower()
            Manager.set_Loan_Status(status, marvel_Bank)
        elif Option == 8:
            print("Exiting.....")
            print("Exit")
            return
...
def adminLogin():
    while True:
        ...
        if marvel_Bank.findAdmin(email):
            adminMenu()
            return
        else:
...
def clientSide():
    while True:
        ...
        if Option == 1:
            print(f"Please Login First")
            email = input("Please Enter Your Email ")
            if marvel_Bank.finduser(email) is not None:
                userMenu(marvel_Bank.finduser(email))
            else:
                print("Email Not Exist")
        elif Option == 2:
...
while True:
    print(f"Wellcome To {marvel_Bank.name}")
